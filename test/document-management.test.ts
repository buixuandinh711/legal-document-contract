...
import { signDocument, toEthersResult } from "../utils/utils";
...
describe("LegalDocumentManager", () => {
  ...
  describe("Submit document", () => {
    it("Should fail to submit document if caller account is not created yet", async () => {
      await expect(
        documentManager
          .connect(other)
          .submitDocument(
    ...
    it("Should fail to submit document if account not active", async () => {
      ...
      ).to.be.revertedWithCustomError(documentManager, "NotDivisionManager");
    });
